	...
	<div class="card-body">
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered text-center" id="list">
                ...
                <?php 
					$i = 1;
					$qry = pg_query($conn, "SELECT i.*, c.name AS category, (COALESCE((SELECT SUM(quantity) FROM stockin_list WHERE item_id = i.id),0) - COALESCE((SELECT SUM(quantity) FROM stockout_list WHERE item_id = i.id),0)) AS available, s.min_stock, s.max_stock FROM item_list i INNER JOIN category_list c ON i.category_id = c.id INNER JOIN wh_stock_notif s ON s.id = 1 WHERE i.delete_flag = 0 ORDER BY i.date_updated DESC");
